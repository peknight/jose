...
trait JsonWebTokenClaimsPlatform { self: JsonWebTokenClaims =>
  ...
  def expectedAudiences(acceptable: String*): Either[Error, Unit] =
    audience.toRight(OptionEmpty).flatMap(audience => interact(audience, acceptable)).label("audience").as(())
  def requireIssuer: Either[Error, Unit] = issuer.toRight(OptionEmpty.label("issuer")).as(())
  def requireSubject: Either[Error, Unit] = subject.toRight(OptionEmpty.label("subject")).as(())
  def requireAudience: Either[Error, Unit] = audience.toRight(OptionEmpty.label("audience")).as(())
