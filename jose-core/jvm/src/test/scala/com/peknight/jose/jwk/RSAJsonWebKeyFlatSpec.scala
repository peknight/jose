...
class RSAJsonWebKeyFlatSpec extends AsyncFlatSpec with AsyncIOSpec:
  ...
  private val rsaJwkWithPrivateKey: String = "{\"kty\":\"RSA\", \"n\":\"ofgWCuLjybRlzo0tZWJjNiuSfb4p4fAkd_wWJcyQoTbj" +
    ...
    "nQ\"}"
  "RSAJsonWebKey" should "pass for parse example with private" in {
    val run =
      for
  ...
  "RSAJsonWebKey" should "pass for from key with private" in {
    val run =
      for
        publicKey <- RSA.publicKey[IO](n, e).asET
  ...
  "RSAJsonWebKey" should "pass for from key with crt private and back and again" in {
  ...
  "RSAJsonWebKey" should "pass for from crt and back with jws appendix A2" in {
  ...
  "RSAJsonWebKey" should "pass for to json with public key only jwk and include private settings" in {
    val run =
      for
        publicKey <- RSA.publicKey[IO](n, e).asET
        jwk <- JsonWebKey.fromPublicKey(publicKey).eLiftET[IO]
